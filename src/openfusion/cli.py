...
Roles: lead (plans/briefs/verifies), sidekick (implements), cua (computer-use agent: e2e tests and other computer-use tasks, optional), reviewer (fresh-context verdict).
...
BUILTIN = {"lead": "claude:opus", "exec": "codex:gpt-5.6-luna", "review": "codex:gpt-5.6-terra", "cua": None, "test_cmd": None}
...
def defaults() -> dict:
    ...
    for k, envk in (("lead", "FUSION_LEAD"), ("exec", "FUSION_EXEC"), ("review", "FUSION_REVIEW"),
                    ("cua", "FUSION_CUA"), ("test_cmd", "FUSION_TEST_CMD")):
...
def add_role_flags(ap: argparse.ArgumentParser, d: dict) -> None:
    ...
    ap.add_argument("--cua", default=d["cua"], help="computer-use agent spec (runs e2e tests and other computer-use tasks), e.g. codex:gpt-5.6-terra or claude:sonnet (default none)")
...
def main(argv: list[str] | None = None) -> None:
    ...
    if a.cmd == "smoke":
        ...
        rows2 = run_smoke(a.lead, a.exec_spec, review)  # smoke has no CUA (toy repo has nothing to drive)
        print("\n" + format_rows(rows2))
    ...
    cua = None if (a.solo or not a.cua or a.cua.lower() == "none") else a.cua
    cfg = RunConfig(task=" ".join(a.task), cwd=Path(a.cwd), lead=a.lead, exec_spec=a.exec_spec, review=review,
                    cua=cua, test_cmd=a.test_cmd, max_review_rounds=a.max_review_rounds, safe_mode=not a.no_safe_mode,
